 ...
 */
class PluginContainer extends \core\base\Quobject {
	/**
	 ...
	 */
	public function addPlugin($name, \core\plugins\PluginInterface $plugin) {
		if (isset($this->plugins[(string) $name])) {
			throw new \core\exceptions\IllegalArgumentException('Plugin named "' . $name . '" already exists in container');
		}
	 ...
	 * @return PluginContainer
	 */
	public function setPlugin($name, \core\plugins\PluginInterface $plugin) {
		...
		foreach ((array) $plugin->declareHooks() as $hook => $method) {
			...
			if (!is_callable($callable)) {
				throw new \core\exceptions\IllegalArgumentException(sprintf(
